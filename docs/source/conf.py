...
project = u'Flutter Capture SDK'
copyright = str(date.today().year) + u', Socket Mobile, Inc'
author = u'Matthew Croak'
...
# print(release)
ver_array = release.split('-')
# The short X.Y version is ver_array[0].
version = ver_array[0]
# Check for rc version. If there, concatenate it to version.
if (len(ver_array) > 1) and (version.count(".") < 2) :
    version += '.' + ver_array[1]
elif (len(ver_array) > 1) and (int(ver_array[1]) > 0) and (version.count(".") == 2) :
    split = version.split(".")
    end = split[len(split)-1].split("-")[0]
    x = int(end)
    y = int(ver_array[1])
    z = x + y 
    version = split[0] + '.' + split[1] + '.' + str(z)
